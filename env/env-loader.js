...
`.trim()
  ...
  functions: {
    ...
    mapping: {
      // Note: OPENAI_API_KEY is also managed as a Firebase secret (see ConfigFunctions.ts)
      'OPENAI_API_KEY': 'OPENAI_API_KEY',
      'OPENAI_FAST_MODEL': 'OPENAI_FAST_MODEL',
  ...
  join: {
    ...
    mapping: {
      ...
      // Sentry (shares the main app's project; tagged with `app: 'join'` at runtime)
      'SENTRY_DSN': 'VITE_SENTRY_DSN',
      'ENVIRONMENT': 'VITE_ENVIRONMENT',
      'APP_VERSION': 'VITE_APP_VERSION',
    },
  ...
  agora: {
    path: path.join(ROOT_DIR, 'apps', 'agora'),
    filename: '.env.local',
    // Map generic vars to Vite-prefixed vars (client-only app)
    mapping: {
  ...
  odyssey: {
    path: path.join(ROOT_DIR, 'apps', 'odyssey'),
    filename: '.env.local',
    // Map generic vars to Vite-prefixed vars (client-only app)
    mapping: {
      'FIREBASE_API_KEY': 'VITE_FIREBASE_API_KEY',
