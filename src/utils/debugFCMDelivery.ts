...
export async function debugFCMDelivery() {
    ...
    onMessage(messaging, (payload) => {
        console.info('%c[FCM MESSAGE RECEIVED]', 'color: green; font-weight: bold', new Date().toISOString());
    ...
    console.error = function(...args) {
        if (args[0]?.toString().includes('FCM') || args[0]?.toString().includes('push') || args[0]?.toString().includes('notification')) {
            console.info('%c[FCM ERROR DETECTED]', 'color: red; font-weight: bold', ...args);
        }
        originalError.apply(console, args);
    };
    
    console.info('%c=== Monitoring active. Send a test message from another browser ===', 'color: purple; font-weight: bold');
