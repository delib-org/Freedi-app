export async function fixChromeServiceWorker() {
    console.info('%c=== FIXING CHROME SERVICE WORKER ===', 'color: red; font-weight: bold; font-size: 20px');
    ...
    registrations.forEach((reg, index) => {
        ...
        console.info('   - Script:', reg.active?.scriptURL || reg.installing?.scriptURL || reg.waiting?.scriptURL || 'None');
...
if (typeof window !== 'undefined') {
    (window as { fixChromeServiceWorker?: typeof fixChromeServiceWorker }).fixChromeServiceWorker = fixChromeServiceWorker;
}
