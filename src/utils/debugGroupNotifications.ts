...
export async function debugGroupNotifications(statementId?: string) {
    console.info('%c=== GROUP NOTIFICATIONS DEBUG ===', 'color: blue; font-weight: bold; font-size: 16px');
    ...
    try {
        ...
        tokensSnapshot.forEach(doc => {
            ...
            console.info(`     Last Update: ${data.lastUpdate?.toDate ? data.lastUpdate.toDate() : data.lastUpdate}`);
        ...
        for (const subDoc of subscriptionsSnapshot.docs) {
            ...
            console.info(`     Notifications: InApp=${subInfo.notifications.inApp}, Push=${subInfo.notifications.push}, Email=${subInfo.notifications.email}`);
        }
