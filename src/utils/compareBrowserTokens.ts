...
export async function compareBrowserTokens() {
    console.info('%c=== BROWSER TOKEN COMPARISON ===', 'color: purple; font-weight: bold; font-size: 16px');
    
    const messaging = getMessaging(app);
    const currentToken = await getToken(messaging, { vapidKey });
    
    console.info('1. Current Browser:', navigator.userAgent.includes('Chrome') ? 'Chrome' : 
...
if (typeof window !== 'undefined') {
    (window as { compareBrowserTokens?: typeof compareBrowserTokens }).compareBrowserTokens = compareBrowserTokens;
}