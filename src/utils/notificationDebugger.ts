...
export async function comprehensiveNotificationDebug() {
	...
	results.firebase = {
		...
		vapidKeyPreview:
			vapidKey && vapidKey !== 'undefined' ? vapidKey.substring(0, 20) + '...' : 'INVALID',
	};
	...
	if ('serviceWorker' in navigator) {
		...
		console.info('Service Workers:', {
			data: JSON.stringify(results.serviceWorkers?.registrations, null, 2),
		});
		...
		if (firebaseSW) {
			...
			const fbSwRegistration =
				await navigator.serviceWorker.getRegistration('/firebase-messaging-sw/');
			if (fbSwRegistration) {
				console.info('✅ Firebase messaging SW found under specific scope:', {
	...
	if (results.browserSupport?.permission === 'default') {
		console.info(
			'2. Request notification permission by calling: notificationService.requestPermission()',
		);
	}
...
if (typeof window !== 'undefined') {
	(
		window as { comprehensiveNotificationDebug?: typeof comprehensiveNotificationDebug }
	).comprehensiveNotificationDebug = comprehensiveNotificationDebug;
}
