...
export async function debugDeploymentNotifications() {
	console.info(
		'%c=== DEPLOYMENT NOTIFICATIONS DEBUG ===',
	...
	console.info(
		'   - VAPID Key:',
		vapidKey === 'undefined' ? 'STRING "undefined"' : vapidKey ? 'Present' : 'NOT SET',
	);
	...
	if (!expectedProjectId || !expectedSenderId) {
		if (currentDomain === 'freedi.tech' || currentDomain === 'delib.web.app') {
			...
			currentDomain === 'wizcol-app.web.app' ||
			currentDomain === 'app.wizcol.com' ||
			currentDomain.endsWith('.wizcol.com') ||
			currentDomain === 'wizcol.com'
		) {
	...
	console.info(
		`   - Project ID match: ${projectIdMatch ? '✅' : '❌'} (app: ${firebaseConfig.projectId}, sw: ${expectedProjectId})`,
	);
	console.info(
		`   - Sender ID match: ${senderIdMatch ? '✅' : '❌'} (app: ${firebaseConfig.messagingSenderId}, sw: ${expectedSenderId})`,
	);
...
if (typeof window !== 'undefined') {
	(
		window as { debugDeploymentNotifications?: typeof debugDeploymentNotifications }
	).debugDeploymentNotifications = debugDeploymentNotifications;
}
