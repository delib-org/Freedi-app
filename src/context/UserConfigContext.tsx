...
const getDirections = (language: string): { dir: Direction; rowDirection: RowDirection } => {
...
export const UserConfigProvider: React.FC<UserConfigProviderProps> = ({ children }) => {
		...
		try {
			...
			if (savedConfig) {
				...
				return {
					...
					learning: {
						evaluation: parsedConfig.learning?.evaluation ?? DEFAULT_LEARNING_SETTINGS.evaluation,
						addOptions: parsedConfig.learning?.addOptions ?? DEFAULT_LEARNING_SETTINGS.addOptions,
	...
	const decrementLearning = useCallback(
		(type: 'evaluation' | 'addOptions' | 'communityVoiceLabels') => {
			setConfig((prev) => {
