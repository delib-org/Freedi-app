...
// Global mock for valibot — applied to all imports across the project.
//
// Delegates to the canonical mock in __mocks__/valibot.ts rather than keeping a
// second copy here. The two had drifted: this factory was missing
// `tupleWithRest`, so every suite that transitively imported a schema using it
// failed to load with "valibot_1.tupleWithRest is not a function". One copy
// cannot drift from itself.
//
// jest.config.json also maps `^valibot$` to that file; this registration is what
// actually takes effect, since setupFilesAfterEach runs after module mapping.
jest.mock('valibot', () => jest.requireActual('../__mocks__/valibot'));
