...
const StreakIndicator: React.FC<StreakIndicatorProps> = ({ count, lastActiveDate, className }) => {
	if (count <= 0) return null;

	const atRisk = lastActiveDate ? isStreakAtRisk(lastActiveDate) : false;

	const classes = clsx('streak-indicator', atRisk && 'streak-indicator--at-risk', className);
