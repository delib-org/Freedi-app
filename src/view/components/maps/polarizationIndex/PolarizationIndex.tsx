					...
					<div className={styles.board}>
						...
						<div className={styles.zones}>
							...
							<div className={`${styles.zone} ${styles['zone--consensus']}`}>
								<span className={styles.zone__icon} role="img" aria-label="consensus">
									&#10004;
								</span>
							...
							<div className={`${styles.zone} ${styles['zone--rejection']}`}>
								<span className={styles.zone__icon} role="img" aria-label="rejection">
									&#10006;
								</span>
							...
							<div className={`${styles.zone} ${styles['zone--polarized']}`}>
								<span className={styles.zone__icon} role="img" aria-label="polarized">
									&#8646;
								</span>
							...
							<div className={`${styles.zone} ${styles['zone--neutral']}`}>
								<span className={styles.zone__icon} role="img" aria-label="neutral">
									&#8596;
								</span>
									...
									<React.Fragment key={axis.questionId}>
										{axis.groups.map((group: Group, i: number) => (
											<div
												...
												style={{
													left:
														showGroups === point.statementId && group.position
															? group.position.x + 'px'
															: point.position
																? point.position.x + 10 + 'px'
																: '10px',
													top:
														showGroups === point.statementId && group.position
															? group.position.y + 'px'
															: point.position
																? point.position.y + 10 + 'px'
																: '10px',
													opacity: showGroups === point.statementId ? 1 : 0,
												}}
											>
												<Tooltip
													content={`${group.option.option} MAD: ${group.mad.toFixed(2)}, Mean: ${group.mean.toFixed(2)}, N: ${group.n}`}
													position={tooltipPosition(group.mad, group.mean)}
												>
													<div
														className={styles.axisGroupPoint}
														style={{ backgroundColor: group.option.color }}
													/>
												</Tooltip>
											</div>
										))}
