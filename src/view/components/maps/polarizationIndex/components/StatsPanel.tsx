		...
		<div className={styles.statsGrid}>
			...
			<div className={styles.statsCard}>
				...
				<div className={styles.statsContent}>
					...
					<div><strong>Last Updated:</strong> {currentStatementData.lastUpdated ? new Date(currentStatementData.lastUpdated).toLocaleString() : 'Unknown'}</div>
				</div>
			</div>
			...
			<div className={styles.axisStatsCard}>
				...
				<div className={styles.statsContent}>
					...
					<div><strong>Groups:</strong> {currentAxis.groups?.length || 0}</div>
					<div><strong>Total Members:</strong> {currentAxis.groups?.reduce((sum: number, g) => sum + (g.numberOfMembers || 0), 0) || 0}</div>
				</div>
			</div>
		</div>
	);
