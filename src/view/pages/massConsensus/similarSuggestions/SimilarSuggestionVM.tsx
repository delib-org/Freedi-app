...
export function useSimilarSuggestions() {
	...
	async function handleSetSuggestionToDB(statement: Statement | GeneratedStatement) {

		try {
			const parentStatement = await getStatementFromDB(parentId)
			if (!parentStatement) throw new Error('Error getting parent statement from DB')
			...
			if (!statement.statementId) {
				...
				const { statementId: newStatementId } = await setStatementToDB({ statement: newStatement, parentStatement })
			...
			navigate(`/mass-consensus/${parentId}/${MassConsensusPageUrls.randomSuggestions}`)
