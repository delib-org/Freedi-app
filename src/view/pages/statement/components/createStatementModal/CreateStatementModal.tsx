...
import {
	...
	DetectedSuggestion,
} from '@/services/multiSuggestionDetection';
...
const CreateStatementModal: FC<CreateStatementModalProps> = ({
	...
	const defaultIsOption =
		parentStatement !== 'top' && parentStatement.statementType === StatementType.option
			? false
			: isOption;
		...
		if (isMultiSuggestionEnabled && isOptionSelected && title.trim().length > 0) {
			...
			try {
				...
				if (result.ok && result.isMultipleSuggestions && result.suggestions.length > 1) {
					...
					const splitSuggestions: SplitSuggestion[] = result.suggestions.map(
						(s: DetectedSuggestion, i: number) => ({
		...
		<div className={styles.createStatementModal}>
			<form className={styles.overlay} onSubmit={onFormSubmit}>
				<div className={styles.modalImage}>
					<img src={isOptionSelected ? newOptionGraphic : newQuestionGraphic} alt="New Statement" />
				</div>
