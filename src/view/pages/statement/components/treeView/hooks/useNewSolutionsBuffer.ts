 ...
 *
 * On first render (or when the list is empty) all children are shown
 * immediately. After that, new arrivals are held in a buffer and a
 * "N new solutions" pill can be shown. Calling `showPending` merges
 * them into the visible set.
 *
 * The current user's own solutions always bypass the buffer.
 *
 * The buffer is flushed automatically when:
 * - `isBufferingActive` becomes false (e.g. sort mode changes)
 * - The parent statement changes (component remount)
 */
export function useNewSolutionsBuffer(
	...
	// IDs the user has "acknowledged" — initially all loaded items
	...
	useEffect(() => {
		if (!isBufferingActive) {
			...
			// Sort mode just changed while buffering stays active — flush
			hasInitializedRef.current = true;
			setAcknowledgedIds(new Set(allChildren.map((c) => c.statementId)));
		}
	...
	// Auto-acknowledge the current user's own solutions so they appear immediately
	useEffect(() => {
	...
	const { visibleChildren, pendingCount } = useMemo(() => {
		// Not buffering or not yet initialized → show everything
