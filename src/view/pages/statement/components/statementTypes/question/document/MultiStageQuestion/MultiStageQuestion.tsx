...
const MultiStageQuestion: FC = () => {
	...
	return (
		<>
			<StageModals showAddStage={showAddStage} setShowAddStage={setShowAddStage} />

			{!hasStages && (
				<EmptyStateSection
					description={
						hasParagraphsContent(statement?.paragraphs)
			...
			{statement.statementSettings?.enableAddNewSubQuestionsButton && (
				<div className={`btns ${styles['add-stage']}`}>
					<button className="btn btn--secondary" onClick={() => setShowAddStage(true)}>
						{t('Add sub-question')}
					</button>
				</div>
			)}
			...
			{!hasStages ? (
				...
				<div className={styles.stagesWrapper}>
					...
					{draggedItem && (
						<DragGhostItem draggedItem={draggedItem} stage={initialStages[draggedItem.index]} />
					)}
