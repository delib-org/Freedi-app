...
describe('Statement Type Validation', () => {
	...
	describe('TYPE_RESTRICTIONS configuration', () => {
		it('should have restrictions defined for option type', () => {
			...
			expect(TYPE_RESTRICTIONS[StatementType.option].disallowedChildren).toContain(
		...
		it('should have restrictions defined for group type', () => {
			...
			expect(TYPE_RESTRICTIONS[StatementType.group].disallowedChildren).toContain(
				StatementType.option,
			);
		});

		it('should have reason messages for restrictions', () => {
			expect(TYPE_RESTRICTIONS[StatementType.option].reason).toBe(
				'Options cannot contain other options',
			);
	...
	describe('Edge cases', () => {
		it('should handle undefined parent gracefully', () => {
			...
			expect(() =>
				isStatementTypeAllowedAsChildren(undefinedParent, StatementType.option),
			).not.toThrow();
		});
		...
		it('should handle null parent gracefully', () => {
			...
			expect(() =>
				isStatementTypeAllowedAsChildren(nullParent, StatementType.option),
			).not.toThrow();
		});
...
describe('Role and Type Helpers', () => {
	...
	describe('isChatMessage', () => {
		it('should return true for statement type', () => {
			expect(
				isChatMessage(StatementType.statement as unknown as Parameters<typeof isChatMessage>[0]),
			).toBe(true);
		});

		it('should return false for option type', () => {
			expect(
				isChatMessage(StatementType.option as unknown as Parameters<typeof isChatMessage>[0]),
			).toBe(false);
		});

		it('should return false for question type', () => {
			expect(
				isChatMessage(StatementType.question as unknown as Parameters<typeof isChatMessage>[0]),
			).toBe(false);
		});
	});
	...
	describe('isMassConsensus', () => {
		it('should return true for massConsensus type', () => {
			expect(
				isMassConsensus(
					QuestionType.massConsensus as unknown as Parameters<typeof isMassConsensus>[0],
				),
			).toBe(true);
		});
		...
		it('should return false for simple type', () => {
			expect(
				isMassConsensus(QuestionType.simple as unknown as Parameters<typeof isMassConsensus>[0]),
			).toBe(false);
		});
	});
});
...
describe('String Manipulation Helpers', () => {
	...
	describe('truncateString', () => {
		it('should truncate long strings', () => {
			expect(truncateString('This is a very long string that needs truncation', 20)).toBe(
				'This is a very long ...',
			);
		});
	...
	describe('getTitle', () => {
		it('should return first line of statement', () => {
			const statement = { statement: 'Title\nDescription here' } as unknown as Parameters<
				typeof getTitle
			>[0];
	...
	describe('getDescription', () => {
		it('should return everything after first line', () => {
			const statement = { statement: 'Title\nLine 1\nLine 2' } as unknown as Parameters<
		...
		it('should return empty for single line', () => {
			const statement = { statement: 'Just a title' } as unknown as Parameters<
				typeof getDescription
			>[0];
...
describe('Array Helpers', () => {
	...
	describe('getLatestUpdateStatements', () => {
		...
		it('should return 0 for empty array', () => {
			expect(getLatestUpdateStatements([] as Parameters<typeof getLatestUpdateStatements>[0])).toBe(
				0,
			);
		});
		...
		it('should return 0 for undefined', () => {
			expect(
				getLatestUpdateStatements(
					undefined as unknown as Parameters<typeof getLatestUpdateStatements>[0],
				),
			).toBe(0);
		});
	});
