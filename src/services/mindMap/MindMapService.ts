...
return cached;
      ...
      // Get root statement
      const rootStatement = state.statements.statements.find(
      ...
      // Get all descendants
      const allDescendants = state.statements.statements.filter(
        s => s.parents?.includes(statementId)
      );

      // Build optimized tree structure
      const tree = this.buildOptimizedTree(
        rootStatement,
        allDescendants,
        maxDepth
      );
...
return data;
   ...
   */
  private populateNodeMap(
    node: MindMapNode,
    map: Map<string, MindMapNode>
  ): void {
...
return null;
  ...
  private async retryLoad(
    ...
    const delay = Math.min(
      MINDMAP_CONFIG.RETRY.INITIAL_DELAY * Math.pow(MINDMAP_CONFIG.RETRY.EXPONENTIAL_FACTOR, attempt - 1),
