...
for(let i=0;i<85000&&gn<58000;i++){const x=(rand()-.5)*145,z=(rand()-.5)*145;if(Math.hypot(x,z-12)<23||(z>3&&Math.abs(x-pathX(z))<1.2))continue;dummy.position.set(x,height(x,z),z);dummy.rotation.set(0,rand()*6.28,0);const s=.5+rand()*1.1;dummy.scale.set(s,.55+rand()*.9,s);dummy.updateMatrix();grass.setMatrixAt(gn,dummy.matrix);grass.setColorAt(gn,new THREE.Color().setHSL(.19+rand()*.055,.23+rand()*.18,.27+rand()*.17));gn++;}grass.count=gn;grass.receiveShadow=true;grass.frustumCulled=false;scene.add(grass);
