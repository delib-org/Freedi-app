...
// ---------- The woven-in mark: a tick is PENDING until the save ----------
step('S1 ticks the accepted idea as woven in — nothing is announced yet');
// Accepting opened the drawer that sits right under the edit box
await s1.waitForSelector('.chat-drawer', { timeout: 15000 });
const drawer = s1.locator('.chat-drawer');
if (!(await drawer.getAttribute('class') ?? '').includes('chat-drawer--open'))
	await drawer.locator('.chat-drawer__head').click();
await s1.waitForSelector('.chat-drawer--open', { timeout: 5000 });
console.log('S1 ACCEPTED-IDEAS DRAWER:', await drawer.locator('.chat-drawer__count').textContent());
await shot(s1, '07c-accepted-drawer');
// The hidden input is pointer-events:none — click the drawn box. Scope to
// rows whose input is enabled: already-implemented rows are checked+disabled.
await drawer
	.locator('.chat-drawer__check:has(.chat-drawer__check-input:not([disabled])) .chat-drawer__check-box')
	.first()
	.click();
await s1.waitForSelector('.chat-drawer__check-input:checked', { timeout: 5000 });
console.log('S1 TICKED THE IDEA ✓');
// The promise of "pending": the suggester hears NOTHING until a real change
await s2.waitForTimeout(2500);
if ((await s2.locator('.celebration').count()) > 0)
	throw new Error('S2 was told "woven in" before S1 saved the proposal');
console.log('S2 GOT NO EARLY ANNOUNCEMENT ✓');

// S1 improves their own proposal in the always-editable box → glitter,
// and the same save announces the ticked idea to its author
await s1
...
// The suggester now hears it — the second, stronger mark of the lifecycle
await s2.waitForSelector('.celebration', { timeout: 20000 });
const wovenMessage = await s2.locator('.celebration__message').textContent();
console.log('S2 CELEBRATION (woven in):', wovenMessage.slice(0, 60));
if (!/נכנס לנוסח|woven into/i.test(wovenMessage))
	throw new Error(`Expected the "woven in" celebration, got: ${wovenMessage}`);
await s2.waitForTimeout(600);
await shot(s2, '07d-celebration-woven-in');
await s2.locator('.celebration button.btn--primary').click();
...
// The chip now carries the LATER mark: accepted → woven in
if ((await helped.locator('.helped__chip--implemented').count()) === 0)
	throw new Error('Helped section missing the woven-in status chip');
