...
/**
 * The bridge zone, in the map's own units.
 *
 * ⚠ These four numbers ARE `.board__bridge-zone` in components.scss, converted
 * from its box to the axes the points are plotted on:
 *   inset-inline-start 28% + width 44% → x ∈ [28,72] → lean ∈ [−0.44, +0.44]
 *   inset-block-end 62% (bottom-based) → y ≥ 62      → percent ≥ 24
 * Change one and change the other, or the dashed box and the cheer stop
...
/**
 * Is this proposal standing in the bridge zone — enough of the class behind
 * it, and both camps behind it fairly evenly?
 *
