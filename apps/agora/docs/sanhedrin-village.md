...
The art is rendered by a second Vite HTML entry at `/prototypes/olive-hill/village.html`. Two approved 2D women replace the rejected procedural Blender villagers: the supplied silver-haired elder at the study house, and the original demo guide at the workshop. `characters-2d.js` is the registry for additional user-supplied characters. Their textured planes have fixed facing directions, preserve image proportions and colors, and hide from edge/reverse views. Entering a station in the standalone tour opens a large portrait with its question; real sessions continue opening the existing activity. The four additional characters are awaiting references. Blender experiments remain archived but are not loaded by the village.
