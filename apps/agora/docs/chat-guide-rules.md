...
| Situation | Guide reaction | Status |
|---|---|---|
| **ACCEPT ("I'll implement")** | The strongest moment in the loop — accepting = *adopting*, so the guide immediately walks the owner to the work: user echo (`chat.accept_echo`), guide line (`chat.accepted_go_improve_1/_2` rotate: "Great choice! Here's your proposal 📘 — weave the idea in"), then the **my-proposal editor opens as the active card quoting ALL accepted ideas above the textarea** (live list of every accepted suggestion; the just-accepted text rides on the card ref until the resolve lands in the snapshot) (💡 `chat.accepted_reminder` + the text, peer-orange accent per I7 — the idea is a classmate's contribution). The student never has to remember the wording. Phase → `improve_mine`, `visited.improveMine` set. The suggester gets the glitter (server notification → celebration, §5). | [IMPLEMENTED 2026-08-04] |
...
| **Guide says something about it in MY chat** | Next time the helper is *at the menu* (their natural turn), the guide's nudge line should be able to say "your idea was woven into a classmate's proposal — go see" (nudge priority just below fresh-feedback). Today the trace lives only in the `helped` branch chips + celebration. | [PLANNED] |
