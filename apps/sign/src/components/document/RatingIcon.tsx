import React from 'react';

/** Material Design thumb-up path (24x24 viewBox) */
const THUMB_UP =
  'M1 21h4V9H1v12zm22-11c0-1.1-.9-2-2-2h-6.31l.95-4.57.03-.32c0-.41-.17-.79-.44-1.06L14.17 1 7.59 7.59C7.22 7.95 7 8.45 7 9v10c0 1.1.9 2 2 2h9c.83 0 1.54-.5 1.84-1.22l3.02-7.05c.09-.23.14-.47.14-.73v-2z';

/** Material Design thumb-down path (24x24 viewBox) */
const THUMB_DOWN =
  'M15 3H6c-.83 0-1.54.5-1.84 1.22l-3.02 7.05c-.09.23-.14.47-.14.73v2c0 1.1.9 2 2 2h6.31l-.95 4.57-.03.32c0 .41.17.79.44 1.06L9.83 23l6.59-6.59c.36-.36.58-.86.58-1.41V5c0-1.1-.9-2-2-2zm4 0v12h4V3h-4z';

interface RatingIconProps {
  rating: number;
  className?: string;
}

/**
 * SVG rating icons for the -1 to +1 satisfaction scale (ported from mass-consensus).
 *
 * -1   = Double thumb down (very unsatisfied)
 * -0.5 = Single thumb down (unsatisfied)
 *  0   = Horizontal dash (neutral)
 * +0.5 = Single thumb up (satisfied)
 * +1   = Double thumb up (very satisfied)
 *
 * Uses `currentColor` so the icon inherits its parent's text color.
 * Uses `width/height: 1em` so it scales with the parent's font-size.
 */
export default function RatingIcon({ rating, className }: RatingIconProps) {
  const svgProps: React.SVGProps<SVGSVGElement> = {
    xmlns: 'http://www.w3.org/2000/svg',
    width: '1em',
    height: '1em',
    fill: 'currentColor',
    className,
    'aria-hidden': true,
    focusable: 'false',
  };

  switch (rating) {
    case 1:
      // Very satisfied: two full-size thumbs up with slight overlap
      return (
        <svg {...svgProps} width="1.75em" viewBox="0 0 42 24">
          <g transform="scale(0.85)">
            <path d={THUMB_UP} />
          </g>
          <g transform="translate(22, 0) scale(0.85)">
            <path d={THUMB_UP} />
          </g>
        </svg>
      );

    case 0.5:
      return (
        <svg {...svgProps} viewBox="0 0 24 24">
          <path d={THUMB_UP} />
        </svg>
      );

    case 0:
      return (
        <svg {...svgProps} viewBox="0 0 24 24">
          <rect x="4" y="10" width="16" height="4" rx="2" />
        </svg>
      );

    case -0.5:
      return (
        <svg {...svgProps} viewBox="0 0 24 24">
          <path d={THUMB_DOWN} />
        </svg>
      );

    case -1:
      // Very unsatisfied: two full-size thumbs down with slight overlap
      return (
        <svg {...svgProps} width="1.75em" viewBox="0 0 42 24">
          <g transform="scale(0.85)">
            <path d={THUMB_DOWN} />
          </g>
          <g transform="translate(22, 0) scale(0.85)">
            <path d={THUMB_DOWN} />
          </g>
        </svg>
      );

    default:
      return null;
  }
}
