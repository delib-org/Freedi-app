...
describe('mapMainAppPathToJoinTarget', () => {
  ...
  it('returns null for out-of-scope screens (vote, mind-map, settings)', async () => {
    expect(
      await mapMainAppPathToJoinTarget(`/statement/${MAIN_ID}/vote`, MAIN_ID),
    ).toBeNull();
    expect(
      await mapMainAppPathToJoinTarget('/statement/q-1/mind-map', MAIN_ID),
    ).toBeNull();
    expect(
      await mapMainAppPathToJoinTarget('/statement/q-1/settings', MAIN_ID),
    ).toBeNull();
  });
  ...
  it('handles trailing slashes / extra whitespace', async () => {
    expect(
      await mapMainAppPathToJoinTarget(`  /statement/${MAIN_ID}/  `, MAIN_ID),
    ).toEqual({ type: 'hub' });
  });
});
...
describe('joinTargetToRoute', () => {
  ...
  it('builds /m/:mid/q/:qid for solutions', () => {
    expect(joinTargetToRoute({ type: 'solutions', questionId: 'q-1' }, MAIN_ID))
      .toBe(`/m/${MAIN_ID}/q/q-1`);
  });

  it('builds /m/:mid/q/:qid/s/:sid for chat', () => {
    expect(
      joinTargetToRoute(
        { type: 'chat', questionId: 'q-1', optionId: 'opt-9' },
        MAIN_ID,
      ),
    ).toBe(`/m/${MAIN_ID}/q/q-1/s/opt-9`);
  });
});
