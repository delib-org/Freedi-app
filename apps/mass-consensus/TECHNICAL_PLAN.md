# Technical Plan: Fast-Loading Mass Consensus Module

## Overview

This Next.js application provides a fast-loading, SSR-optimized interface for the Freedi mass consensus building. It's designed to load in < 1 second and provide an exceptional user experience for evaluating and submitting solutions.
...
jobs:
  deploy:
    ...
    steps:
      ...
      - run: cd apps/mass-consensus && npm ci
      - run: cd apps/mass-consensus && npm run build
      - run: cd apps/mass-consensus && npm test
...
5. **Embeddable Widget**: Share mass consensus building on other sites
...
This fast-loading module successfully demonstrates that we can provide a dramatically better user experience for mass consensus building while maintaining compatibility with the existing Freedi data model. The SSR approach with Next.js provides the performance boost we need, and the anonymous-first approach removes friction for new users.
