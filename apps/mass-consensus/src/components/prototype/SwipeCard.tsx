 ...
 */
const SwipeCard = forwardRef<SwipeCardHandle, SwipeCardProps>(({ text, onSwipe, onNext }, ref) => {
  ...
  useImperativeHandle(ref, () => ({
    throwCard: (rating: number) => {
      const direction = rating > 0 ? 'right' : 'left';
  ...
  const handleAutoThrow = (delta: number) => {
    ...
    console.info('Throwing card with rating:', rating, 'direction:', direction);
  ...
  return (
    <div
      ...
      style={{
        ...
        willChange: isDragging || isThrowing || isEntering ? 'transform, opacity' : 'auto',
      }}
      // Touch events (mobile)
      onTouchStart={(e) => {
