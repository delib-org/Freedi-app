...
- Produce exactly these 7 sections, in this order, with these ids:
  ...
  6. "psychologicalInsights" — apply named, evidence-based behavioral effects ONLY where the data supports them, for example: loss aversion visible in rejection-reason language; social proof effects of visible support counts; the IKEA/ownership effect — commenters whose input is visibly incorporated become supporters; the peak-end rule if attention or support sags in the closing paragraphs. Explain each effect in one plain sentence before applying it.
