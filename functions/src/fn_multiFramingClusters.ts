...
export async function generateMultipleFramings(
  ...
  try {
    ...
    if (!statementId || typeof statementId !== "string") {
      res
        .status(400)
        .send({ error: "Invalid input: statementId is required", ok: false });
    ...
    if (!topic || !topic.statementId) {
      res
        .status(400)
        .send({ error: "Invalid input: topic not found", ok: false });
    ...
    if (!descendants || descendants.length === 0) {
      logger.info(
        "No descendants found for the given statementId:",
        statementId
      );
      res.status(200).send({
    ...
    const simpleDescendants: SimpleDescendant[] = descendants.map(
      (descendant) => ({
    ...
    const prompt = buildMultiFramingPrompt(
      topic,
      simpleDescendants,
      maxFramings
    );
    ...
    const createdFramings = await createFramingsInDatabase(
      topic,
      descendants,
      aiFramings
    );
...
export async function requestCustomFraming(
  ...
  try {
    ...
    if (!customPrompt || typeof customPrompt !== "string") {
      res
        .status(400)
        .send({ error: "Invalid input: customPrompt is required", ok: false });
    ...
    if (!userId || typeof userId !== "string") {
      res
        .status(400)
        .send({ error: "Invalid input: userId is required", ok: false });
    ...
    await db
      .collection(FRAMING_COLLECTIONS.framingRequests)
      .doc(requestId)
      .set(framingRequest);
    ...
    const prompt = buildCustomFramingPrompt(
      topic,
      simpleDescendants,
      customPrompt
    );
...
export async function getFramingsForStatement(
  ...
  try {
    ...
    const framings: Framing[] = framingsSnapshot.docs.map(
      (doc) => doc.data() as Framing
    );
...
export async function getFramingClusters(
  ...
  try {
    ...
    if (!framingId || typeof framingId !== "string") {
      res
        .status(400)
        .send({ error: "Invalid input: framingId is required", ok: false });

      return;
    }

    // Get framing
    const framingDoc = await db
      .collection(FRAMING_COLLECTIONS.framings)
      .doc(framingId)
      .get();
    ...
    const clustersWithOptions = await Promise.all(
      clusters.map(async (cluster) => {
        ...
        const options: Statement[] = optionsSnapshot.docs.map(
...
async function createSingleFraming(
  ...
  for (const group of aiFraming.groups) {
    ...
    for (const statementRef of group.statements) {
      ...
      const originalStatement = descendants.find(
        (d) => d.statementId === statementRef.statementId
      );
      if (originalStatement) {
        const optionRef = db
          .collection(Collections.statements)
          .doc(statementRef.statementId);
  ...
  logger.info(
    `Created framing ${framingId} with ${clusterIds.length} clusters`
  );
...
export async function deleteFraming(
  ...
  try {
    ...
    logger.info(
      `Deleted framing ${framingId} with ${framing.clusterIds.length} clusters`
    );
