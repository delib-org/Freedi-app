 ...
 */
export const suggestClusterTitle = onCall({ region: functionConfig.region }, async (request) => {
	...
	try {
		...
		const { title, description } = await generateGroupedTitle(memberTexts, parent.statement ?? '');
