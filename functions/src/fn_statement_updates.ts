...
export const updateParentOnChildCreate = onDocumentCreated(
  ...
  async (event) => {
    try {
      ...
      logger.info(
        `New child statement created, updating parent ${newStatement.parentId}`
      );
  ...
  async (event) => {
    try {
      ...
      if (JSON.stringify(beforeCopy) === JSON.stringify(afterCopy)) {
        logger.info(
          "Skipping update - appears to be triggered by parent update function"
        );
      ...
      logger.info(
        `Child statement content changed, updating parent ${after.parentId}`
      );
...
async function updateParentWithLatestChildren(parentId: string) {
  try {
    ...
    const lastSubStatements: SimpleStatement[] = subStatementsQuery.docs.map(
      (doc) => {
    ...
    logger.info(
      `Updated parent ${parentId} with ${lastSubStatements.length} sub-statements`
    );
...
async function updateParentSubscriptions(
  ...
  try {
    ...
    logger.info(
      `Updating ${subscriptionsQuery.size} subscriptions for statement ${statementId}`
    );
    ...
    await batch.commit();
    logger.info(
      `Successfully updated ${subscriptionsQuery.size} subscriptions`
    );
  } catch (error) {
    logger.error(
      `Error updating subscriptions for statement ${statementId}:`,
      error
    );
  }
}
