...
describe('isManualCluster', () => {
	it('flags an admin cluster with a locked title', () => {
		expect(isManualCluster(make({ isCluster: true, titleLockedByCreator: true }))).toBe(true);
		...
		expect(
			isManualCluster(make({ isCluster: true, titleLockedByCreator: true, integratedOptions: [] })),
		).toBe(true);
	});

	it('does NOT flag auto-formed synth clusters (no title lock)', () => {
		expect(isManualCluster(make({ isCluster: true }))).toBe(false);
		expect(isManualCluster(make({ isCluster: true, derivedByPipeline: 'synthesis' }))).toBe(false);
		expect(isManualCluster(make({ isCluster: true, integratedOptions: ['a', 'b'] }))).toBe(false);
	});
