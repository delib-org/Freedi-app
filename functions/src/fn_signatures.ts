...
async function onCreateSignature(signature: Signature) {
	try {
		...
		await db.runTransaction(async (transaction) => {
			const documentSignatureRef = db
				.collection(Collections.documentsSigns)
				.doc(documentId);
			...
			if (!documentSignatureDB.exists) {
				...
				const documentSignature: DocumentSigns = {
					...
					totalSignaturesLevel:
						signed === SignatureType.signed ? levelOfSignature : 0,
				};
				transaction.set(documentSignatureRef, documentSignature);
			} else {
				const documentSignature = documentSignatureDB.data() as DocumentSigns;
				const sumSignedAndRejected =
					documentSignature.signed + documentSignature.rejected;
				...
				documentSignature.totalSignaturesLevel +=
					signed === SignatureType.signed ? levelOfSignature : 0;
				documentSignature.avgSignatures =
					sumSignedAndRejected > 0
						? documentSignature.totalSignaturesLevel / sumSignedAndRejected
						: 0;
		...
		await db.runTransaction(async (transaction) => {
			...
			if (documentSignatureDB.exists) {
				...
				documentSignature.totalSignaturesLevel -=
					signed === SignatureType.signed ? levelOfSignature : 0;
				documentSignature.avgSignatures =
					documentSignature.totalSignaturesLevel / documentSignature.viewed;
				transaction.update(documentSignatureRef, documentSignature);
			}
		});
	} catch (error) {
...
async function onUpdateSignature(
	signatureBeforeData: Signature,
	signatureAfterData: Signature
) {
	try {
		const { signed: signedBefore, levelOfSignature: levelOfSignatureBefore } =
			signatureBeforeData;
		const {
			documentId,
			signed: signedAfter,
			levelOfSignature: levelOfSignatureAfter,
		} = signatureAfterData;

		const diffSigned =
			(signedAfter === SignatureType.signed ? 1 : 0) -
			(signedBefore === SignatureType.signed ? 1 : 0);
		const diffRejected =
			(signedAfter === SignatureType.rejected ? 1 : 0) -
			(signedBefore === SignatureType.rejected ? 1 : 0);
		...
		await db.runTransaction(async (transaction) => {
			...
			const documentSignatureDB = await transaction.get(documentSignatureRef);
			if (!documentSignatureDB.exists)
				throw new Error('Document signature not found');

			const documentSignature = documentSignatureDB.data() as DocumentSigns;

			const sumApprovedAndRejected =
				documentSignature.signed + documentSignature.rejected;
