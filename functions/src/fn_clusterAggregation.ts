...
async function calculateClusterAggregation(
  ...
  const evaluationsByUser = new Map<
    string,
    { evaluations: number[]; userId: string }
  >();
  ...
  evaluationsByUser.forEach((userData) => {
    ...
    const userAverage =
      userData.evaluations.reduce((sum, val) => sum + val, 0) /
  ...
  const averageClusterConsensus =
    uniqueEvaluatorCount > 0 ? totalScore / uniqueEvaluatorCount : 0;
  ...
  for (const optionId of optionIdsArray) {
    const count = allEvaluations.filter(
      (e) => e.statementId === optionId
    ).length;
...
export async function recalculateClusterAggregation(
  ...
  try {
    ...
    if (!clusterId || typeof clusterId !== "string") {
      res
        .status(400)
        .send({ error: "Invalid input: clusterId is required", ok: false });
...
export const onEvaluationChangeInvalidateCache = onDocumentWritten(
  ...
  async (
    event: FirestoreEvent<Change<FirebaseFirestore.DocumentSnapshot> | undefined>
  ) => {
    try {
  ...
  try {
    ...
    const averageConsensus =
      aggregations.length > 0
        ? aggregations.reduce((sum, a) => sum + a.averageClusterConsensus, 0) /
          aggregations.length
        : 0;
