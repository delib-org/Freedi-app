...
async function updateStatementEvaluation(
  ...
  const { statementId, evaluationDiff, action, newEvaluation, oldEvaluation } =
    props;
  ...
  try {
    ...
    //Calculating the Squared diff
    const squaredDiff = calcSumSquaredDiff({
      action,
      newEvaluation,
      oldEvaluation,
    });
...
async function ensureAverageEvaluationForAllOptions(
  ...
  try {
    ...
    for (const doc of optionsSnapshot.docs) {
      ...
      if (data.evaluation?.averageEvaluation === undefined) {
        needsUpdate = true;
...
async function updateStatementInTransaction(
  ...
  await db.runTransaction(async (transaction) => {
    ...
    if (
      ...
      statementData.evaluation?.averageEvaluation === undefined
    ) {
      ...
      if (statementData.evaluation) {
        ...
        statementData.evaluation.averageEvaluation =
          statementData.evaluation.numberOfEvaluators > 0
            ? statementData.evaluation.sumEvaluations /
              statementData.evaluation.numberOfEvaluators
            : 0;
        statementData.evaluation.sumSquaredEvaluations ??= 0;
      } else {
...
function calculateEvaluation(
  ...
  if (statement.evaluation) {
    ...
    // update sumSquaredEvaluations incrementally
    evaluation.sumSquaredEvaluations =
      (evaluation.sumSquaredEvaluations || 0) + (squaredDiff || 0);
  } else {
    ...
    // set sumSquaredEvaluations for new
    evaluation.sumSquaredEvaluations = squaredDiff || 0;
  }
...
function calcAgreement(
  ...
  if (numberOfEvaluators <= 1)
    return sumEvaluations / Math.max(numberOfEvaluators, 1);

  const mean = sumEvaluations / numberOfEvaluators;
  const variance =
    (sumSquaredEvaluations - sumEvaluations ** 2 / numberOfEvaluators) /
    (numberOfEvaluators - 1);

  const sd = Math.sqrt(Math.max(variance, 0));
  const sem = sd / Math.sqrt(numberOfEvaluators);

  return mean - sem;
}
function calcSumSquaredDiff({
  action,
  newEvaluation,
  oldEvaluation,
}: {
  ...
  switch (action) {
    case ActionTypes.new:
      return newEvaluation ** 2;
    case ActionTypes.delete:
      return -(oldEvaluation ** 2);
    case ActionTypes.update:
      return newEvaluation ** 2 - oldEvaluation ** 2;
    default:
      return 0;
  }
}
...
async function updateParentStatementWithChosenOptions(
  ...
  try {
    ...
    const chosenOptions = await choseTopOptions(
      parentId,
      parentStatement.resultsSettings
    );
...
async function updateParentTotalEvaluators(parentId: string): Promise<void> {
  try {
    ...
    logger.info(
      `Updated parent ${parentId} with ${totalUniqueEvaluators} total unique evaluators`
    );
  } catch (error) {
...
async function choseTopOptions(
  ...
  try {
    ...
    const chosenOptions = await getOptionsUsingMethod(
      parentId,
      resultsSettings
    );
...
async function clearPreviousChosenOptions(
  parentId: string | undefined
): Promise<void> {
  try {
    ...
    const batch = db.batch();
    for (const doc of previousChosenDocs.docs) {
...
function getSortedOptions(
  ...
  return statements.sort(
    sortComparisons[resultsBy] || sortComparisons[ResultsBy.consensus]
  );
}
...
async function getOptionsUsingMethod(
  ...
  const { numberOfResults, resultsBy, cutoffBy, cutoffNumber } =
    resultsSettings;
