...
import { checkForInappropriateContent, generateTitleAndDescription } from './services/ai-service';
import {
	getUserStatements,
	...
	try {
		...
		if (cachedResponse) {
			...
			const isFallbackPattern =
				generatedDescription?.startsWith('הצעה:') ||
			...
			if (needsRegeneration) {
				try {
					...
					logger.warn('Failed to generate title/description for cached response', {
						error: genError,
					});
					...
					if (!generatedTitle || !generatedDescription) {
						...
						generatedDescription = isHebrew
							? `הצעה זו מציעה: ${userInput}`
							: `This suggestion proposes: ${userInput}`;
					}
				}
			}
		...
		await saveCachedSimilarityResponse(statementId, userInput, creatorId, responseData, threshold);
...
async function fetchDataAndProcess(
	...
	try {
		...
		logger.info(
			`Processing similarity check with ${subStatements.length} existing statements, threshold: ${threshold}`,
		);
		...
		logger.info(
			`Similarity search complete: ${similarStatementIds.length} results via ${searchMethod}`,
		);
