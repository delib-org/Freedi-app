...
| `4-6-2026-41-20-4-singleton-validation` | production: UMAP→DBSCAN + twoTierJudge | Lone statement (archetype C/F) FAIL — 9/10 synths pure + 2 clean topics, but the off-topic statement is absorbed into the nearest synth (14/18). Finding: bulk path does not reliably leave a lone outlier unclustered (UMAP proximity + judge keep-≥80%). |
