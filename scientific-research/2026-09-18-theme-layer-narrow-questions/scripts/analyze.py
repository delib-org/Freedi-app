...
d=json.load(open('q.json')); ch={c['statementId']:c for c in d['children']}
...
for _,r in fan.iterrows():
  c=ch[r.id]; kind='original' if not c.get('isCluster') else f"machine {c['derivedByPipeline']} cluster{' (hidden)' if c.get('hide') else ''}"
