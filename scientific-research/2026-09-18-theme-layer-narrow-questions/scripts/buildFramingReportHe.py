...
SOURCE = sys.argv[3] if len(sys.argv) > 3 else 'dry'
...
e = html.escape
if SOURCE == 'prod':
    SRC_NOTE = 'מצב המכונה הוא מצב הייצור של השאלה ב־18.9.2026 בשעה 15:44, אחרי פריסת התיקון לשכבת הנושאים, בנייה מחדש מאפס (״Re-cluster from scratch״) והתייצבות של סבבי הפיצול והאיחוד האוטומטיים.'
    SRC_METHOD = 'תמונת מצב הייצור מ־18.9.2026 בשעה 15:44 (snapshot-8), אחרי בנייה מחדש מאפס עם התיקון לשכבת הנושאים וההתייצבות של הסבבים האוטומטיים (splitThemes, consolidateThemes).'
else:
    SRC_NOTE = 'מצב המכונה הוא מצב הייצור של השאלה מ־18.9.2026 אחרי סבב הפיצול והאיחוד של שכבת הנושאים (הרצה יבשה עם המודל האמיתי, ללא כתיבה למסד הנתונים). זהו המצב שהמערכת תייצר בייצור לאחר הפריסה.'
    SRC_METHOD = 'תמונת מצב הייצור מ־18.9.2026 (18 קבוצות מיזוג, 2 נושאים) אחרי הרצה יבשה של סבב הפיצול ואחריו סבב האיחוד של שכבת הנושאים, כפי שהם ממומשים בקוד המערכת (splitThemes, consolidateThemes).'
...
<p class="note">החלוקה של פאני: הקובץ statements_full.xlsx (142 שורות; 114 היגדי משתתפים, 27 היגדי אשכול מהריצה הקודמת של המכונה שהוחרגו, ושורה אחת עם תווית בלי היגד). תווית שנייה, כשקיימת, נחשבת כהסכמה. חלוקת המכונה: {SRC_METHOD} הסקריפטים: scoreVsFanny.py, splitDryRun.ts, consolidateDryRun.ts, buildFramingReportHe.py בתיקיית המחקר של 18.9.2026. המדדים: דיוק וכיסוי על זוגות היגדים ומדד ARI כשהיגד לא משויך נחשב אשכול של אחד.</p>
